...
Load preferences from memory. For OpenClaw, store in `memory/wwatcher_preferences.json`. If any filter fails, silently skip (unless `debug: true`, then log the skip reason).
...
| OpenClaw preferences | `~/.openclaw/workspace/memory/wwatcher_preferences.json` |
